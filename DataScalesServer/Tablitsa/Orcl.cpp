...
bool COrcl::sendDataFixWeight(String^ nom, String^ name, Int32 tara, Int32 stabil, Int32 zero, Int32 kg, Int32 tonn, String^ ves, String^ ip, String^ Weigth_Hour, String^ RawData, String^ Fixed, String^ FixedTime) {
		if (_OC == nullptr) {
		_OC = gcnew OracleConnection(_connectionString);
		_OC->Open();
	...
	if (bool::Parse(Fixed) )
		IFix = 1;
	OracleCommand^ cmd = gcnew OracleCommand("VESY_ZAP_PACK.save_ves_fix", _OC);
	cmd->CommandType = System::Data::CommandType::StoredProcedure;
	...
	cmd->Parameters->Add("vesy_cur_user_", Oracle::DataAccess::Client::OracleDbType::Varchar2)->Value = _user;
	cmd->Parameters->Add("vesy_err_code_", Oracle::DataAccess::Client::OracleDbType::Varchar2)->Value = 0;
	cmd->Parameters->Add("vesy_error_", Oracle::DataAccess::Client::OracleDbType::Varchar2)->Value = 0;
	cmd->Parameters->Add("vesy_dnsname_", Oracle::DataAccess::Client::OracleDbType::Varchar2)->Value = _host;//strHostName;
	cmd->Parameters->Add("vesy_fixed_", Oracle::DataAccess::Client::OracleDbType::Int16)->Value = IFix;;
	cmd->Parameters->Add("vesy_datetime_", Oracle::DataAccess::Client::OracleDbType::Date)->Value = DateTime::Parse(FixedTime);
	cmd->Parameters->Add("vesy_raw_", Oracle::DataAccess::Client::OracleDbType::Varchar2)->Value = RawData;
	...
	{ 
		...
		_log->sendOracle("Посылка в БД: " + e->ToString());
	}
	...
	if (_log != nullptr) 
		_log->sendOracle("Посылка в БД: " + ves + " Стаб: " + stabil.ToString());
